...
**Pipeline structure.** Figure 1 shows **PL_BRONZE_INCIDENTS**, which contains a single Dataflow activity.

<img width="1036" height="550" alt="image" src="https://github.com/user-attachments/assets/56691bc4-1e74-4f89-9ccb-1f2fe1fdb4f4" />

*Figure 1 – PL_BRONZE_INCIDENTS (pipeline overview).*

**Activity “Load Incidents”.** Invokes the dataflow **DF_Bronze_Incidents_Append** in the thesis workspace.

Inside **DF_Bronze_Incidents_Append**, the following queries are defined:

1. **Year-specific queries (Original folder).**
   - `gopi2020subseccao.xlsx`
   - `gopi2021subseccao.xlsx`
   - `gopi2022subseccao.xlsx`
   - `ocorrencias-2023.xlsx`
   - `ocorrencias-2024.xlsx`

   Each query:
   - Connects to one Excel workbook delivered by Lisboa Aberta;
   - Promotes the first row to headers;
   - Applies basic type casting to the original columns (`dt_registo`, `Freguesia`, `area`, `tipo`, `Subseccao`, `Longitude_Subseccao`, `Latitude_Subseccao`).

2. **Query Append Data.**
   - **Step A1 – Append.** Combines the five year-specific queries with `Table.Combine`, yielding a vertically concatenated incident table covering all available years.
   - **Step A2 – Column renaming for staging.** The key business columns are renamed to English, analysis-oriented names:
...
3. **Output query `tbl_bronze_incidents_raw`.**
   - Outputs the appended incidents to the Lakehouse as `tbl_bronze_incidents_raw` with the original column names, preserving the raw schema for audit and potential re-processing.
   - No renaming or row filtering is applied beyond type checks.

4. **Output query `stg_incidents_appended`.**
   - Outputs the renamed version of Append Data to the Lakehouse as `stg_incidents_appended`.
   - This staging table is the single source for all Silver-level transformations (dimensional modelling and fact construction). It keeps every row of the raw data while providing stable, English-friendly column names for subsequent transformations.

The destination of the dataflow is the Fabric Lakehouse associated with the thesis workspace, ensuring that both Bronze tables are stored as Delta files with schema preservation and time-travel support.
...
*Figure 2 – PL_BRONZE_WEATHER (pipeline overview).*

**Notebook structure.** PL_BRONZE_WEATHER consists of a Notebook activity (e.g., `NB_Bronze_Weather_OpenMeteo`) orchestrating **six** phases:

**Phase 0 – Configuration and imports.**
- Defines `TARGET_TABLE = "tbl_bronze_weather_raw"`.
- Specifies the daily variables to retrieve (`temperature_2m_max`, `temperature_2m_min`, `precipitation_sum`, `wind_speed_10m_max`), the target time zone (Europe/Lisbon), retry strategy, batching size, and precision for latitude/longitude.
- Imports PySpark, requests, pandas, numpy, and concurrency utilities, and defines `ensure_col_order_and_types` to enforce a canonical column order in the final pandas DataFrame.

**Phase 1 – Date window from DimDate.**
- Reads `dim_date_silver` and selects the `Registration Date` column as a date.
- Computes `start_date` and `end_date` as the minimum and maximum dates available, and derives `N_DAYS` as the number of days in this interval.
- Logs the temporal window that will be requested from Open-Meteo.
- This guarantees temporal alignment between incident workloads and weather, which is essential for BQ5.* analyses.

**Phase 2 – Coordinates per parish.**
- Reads `dim_location_silver`.
- Validates that the table contains `Parish Name`, `Latitude_Centroid`, and `Longitude_Centroid`.
- Normalises parish names (upper + trim) and casts coordinates to double.
- Drops nulls and duplicates, producing a list of coordinates of the form `(latitude, longitude, parish)` for all target parishes.
- Logs the number of parishes and the expected number of rows (parishes × `N_DAYS`).

**Phase 3 – HTTP session and `fetch_daily` function.**
- Configures an HTTP session with retry logic (exponential backoff, respect for `Retry-After` headers, and handling of transient 5xx errors).
- Defines `fetch_daily(lat, lon, freg)` which:
...
- Uses a `ThreadPoolExecutor` to call `fetch_daily` in parallel for batches of parishes, respecting a given batch size and cooldown between batches.
- Accumulates all partial DataFrames into `frames`, tracks rows obtained and expected, and logs progress (coordinates processed, rows retrieved, elapsed time, and estimated time remaining).
- Raises an error if no data are returned, ensuring robustness against misconfiguration.

**Phase 5 – Full load into `tbl_bronze_weather_raw`.**
- Concatenates all frames into `all_pdf`, enforces column order and types, and creates a Spark DataFrame with an explicit schema (date and metric types).
- Normalises parish names (upper + trim), rounds coordinates to the same scale as `dim_location_silver`, drops null parish/date combinations, and removes duplicate `(date, parish)` pairs.
- Writes the result as a Delta table `tbl_bronze_weather_raw` in overwrite mode and executes an `OPTIMIZE … ZORDER BY (date)` statement for query performance.
...
The Silver tier transforms the Bronze tables into conformed dimensions and facts. Two pipelines implement this stage: **PL_SILVER_INCIDENTS** and **PL_SILVER_WEATHER**.

### PL_SILVER_INCIDENTS – Dimensionalisation of incidents

**Purpose.** Derive the conformed dimensions **DimDate**, **DimLocation**, and **DimIncidentType**, and the incident fact table **FactIncidentDaily**, from the staging incidents table `stg_incidents_appended`. This directly implements Steps 2–4 of Kimball’s method and supports BN1–BN4 and BN6–BN7.

**Pipeline structure.** PL_SILVER_INCIDENTS invokes a dataflow grouping the transformations into four queries, as shown in Figure 4:

<img width="945" height="602" alt="image" src="https://github.com/user-attachments/assets/f3a75077-c4cf-408b-ac3b-09afa6e5f4d5" />

*Figure 3 – PL_SILVER_INCIDENTS (dataflow orchestration overview).*
...
#### Activity Load `dim_location` – Dataflow `DF_Silver_DimLocation`
...
- Deduplicates and sorts parishes alphabetically to obtain deterministic ordering.
...
#### Activity Load `dim_incident_type` – Dataflow `DF_Silver_DimIncidentType`
...
#### Activity Load `fact_incident_daily` – Dataflow `DF_Silver_FactIncidentDaily` (**Option A: event-level storage**)

- Reads `stg_incidents_appended` from the Lakehouse as the **event-level incident staging table**, and reads `dim_location_silver` and `dim_incident_type_silver` as lookup dimensions.
- Types core columns (`Registration Date`, `Parish Name`, `Area`, `Type`, `Subsection Longitude`, `Subsection Latitude`) and normalises text fields (upper + trim, PT culture) so that natural keys match dimension tables.
...
- Joins with `dim_incident_type_silver` on (`Area`, `Type`) to obtain `sk_incident_type`, using the harmonised incident taxonomy.
...
- Outputs the result as the incident fact table `fact_incident_daily_silver`.

At this stage, each row corresponds to a single de-normalised incident occurrence; **daily and higher-level workloads** (e.g., incidents per day × parish × type) are obtained in the semantic model through DAX measures and visual grain, while Gold remains a publication layer over the same conformed grain.
...
*Figure 4 – PL_SILVER_WEATHER (pipeline overview).*
...
- Outputs the result as the weather fact table `fact_weather_daily_silver`.
...
*Figure 5 – PL_GOLD_PUBLISH (pipeline overview).*

#### Activity `Load_dim_date`

- **Source:** Lakehouse table `dim_date_silver` (Tables).
- **Destination:** Lakehouse table `dim_date`, action Overwrite.
- **Mapping / steps:**
  - Performs a 1-to-1 mapping of all calendar attributes:
    `sk_date`, `Registration Date`, `Year`, `Month`, `Day`, `Trimester`,
    `Semester`, `Is Last Year`, `Is Last Year-1 Until Last Period`, `Week Day`.
  - No additional calculations are applied in Gold; uniqueness on `sk_date` and time-intelligence flags were already enforced in Silver.
...
- **Mapping / steps:**
  - Copies the surrogate key and descriptive attributes:
...
- **Source:** `dim_incident_type_silver`.
- **Destination:** `dim_incident_type`, action Overwrite.
- **Mapping / steps:**
  - Copies taxonomy attributes:
    `sk_incident_type`, `Area`, `Type`, `bk_incident_type`.
  - Category labels are already normalised in Silver, so Gold simply exposes them for the semantic model.
...
- **Mapping / steps:**
  - Copies the surrogate keys and identifiers:
    `sk_fact_incident`, `sk_date`, `sk_location`, `sk_incident_type`, `bk_fact_incident`.
  - No aggregation is performed: each row in `fact_incident_daily` continues to represent a single incident occurrence.
  - Referential integrity was enforced upstream in Silver, so Gold remains a thin publication layer.
- **Output:** Gold incident fact table `fact_incident_daily`.

Daily workloads are obtained in the semantic model by DAX measures (e.g., `Incidents = COUNTROWS ( fact_incident_daily )`).
...
- **Mapping / steps:**
  - Copies the surrogate keys and daily weather indicators:
    `sk_fact_weather`, `sk_date`, `sk_location`,
    `temperature_2m_max`, `temperature_2m_min`,
    `precipitation_sum`, `wind_speed_10m_max`,
    `Is_Rainy_Day`, `bk_fact_weather`.
  - No aggregation is applied in Gold; each row corresponds to a single parish–day produced by the notebook and conformed in Silver.
- **Output:** Gold weather fact table `fact_weather_daily`, aligned with the same Date and Location dimensions as incidents.
...
- Avoids race conditions where the dataset might refresh while one of the copies is still in progress.
...
- **Steps:**
  - Triggers a refresh of the Direct Lake semantic model bound to the Gold tables.
  - The dataset defines relationships:
...
- **Link to measures.** Detailed definitions of baseline, anomaly and comparison measures (BN2–BN5) are provided in the Measures Design subsection. From the Gold-tier perspective, PL_GOLD_PUBLISH guarantees that these measures operate over clean, conformed star-schema tables with stable keys and a clear analytical grain enforced via the semantic model.

---

## Master orchestration

To avoid managing multiple independent refreshes, the Medallion pipelines are wrapped by a single orchestration pipeline, **PL_MASTER** (Figure 7). This pipeline is responsible for running all stages of the ELT process in the correct order and thus provides a single scheduling and monitoring point for the whole artefact.

<img width="945" height="210" alt="image" src="https://github.com/user-attachments/assets/cf65fa42-9bb1-454c-b195-604588ef6060" />

*Figure 6 – PL_MASTER (end-to-end orchestration).*

The structure of PL_MASTER is deliberately simple and transparent:

- **Activity Invoke PL_BRONZE_INCIDENTS.**  
  Triggers ingestion and append of Na Minha Rua LX incident files into `tbl_bronze_incidents_raw` and derivation of the staging table `stg_incidents_appended`.

- **Activity Invoke PL_SILVER_INCIDENTS.**  
  Runs Silver conformance for incidents, producing `dim_date_silver`, `dim_location_silver`, `dim_incident_type_silver`, and `fact_incident_daily_silver`.

- **Activity Invoke PL_BRONZE_WEATHER.**  
  After incident Silver is available (including DimDate and DimLocation), executes the notebook-driven extraction of historical weather, landing it as `tbl_bronze_weather_raw`.

- **Activity Invoke PL_SILVER_WEATHER.**  
  Conforms the Bronze weather data to the same DimDate and DimLocation used by incidents, generating `fact_weather_daily_silver`.

- **Activity Invoke PL_GOLD_PUBLISH.**  
  Copies Silver tables into Gold star-schema tables (`dim_date`, `dim_location`, `dim_incident_type`, `fact_incident_daily`, `fact_weather_daily`) and refreshes the Direct Lake semantic model.

The control flow is strictly sequential: each activity starts only after the previous one has succeeded. This design keeps failure diagnosis straightforward (errors are localised to a specific stage) and ensures that downstream tables are never refreshed against partially updated upstream data, which is critical for the integrity of baselines and anomaly measures.

Operationally, only PL_MASTER is scheduled; individual Bronze, Silver, and Gold pipelines are treated as reusable building blocks that can also be run ad hoc for debugging or backfill purposes.
